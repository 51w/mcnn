...
void InnerProduct::SetUp(Tensor& Input, Tensor& Output) 
{
...
void InnerProduct::Reshape(Tensor& Input, Tensor& Output)
{
	// No Support
}

void InnerProduct::Run(Tensor& Input, Tensor& Output)
{
	...
	if(bias_term) bias_data = this->blobs_[1]->mutable_cpu_data();
